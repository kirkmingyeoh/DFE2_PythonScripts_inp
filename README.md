...
0) **DFE2_0_UserInput.py** - This user input script is to specify the user-defined inputs, such as the names of the macroscale, microscale RVE and output Direct FE2 input files. Users can also specify additional information such as the number of integration points per macroscale element, as well as tolerance for floating point comparisons. This input script is used with and will be called by the other Python scripts to read the user-defined inputs. 
